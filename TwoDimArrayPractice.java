	   ...
	   for (int i = 0; i < intArray.length; i++) {
			...
			if (i == row) {
				for (int column = 0; column < intArray[i].length; column++) {
					...
					System.out.print(intArray[i][column] + " ");
					animate( row, column );  // needed to create visual feedback
				}
			}
			else {
   ...
   */
   public int findMinimum( int column )
   {
    ...
    // Write your code here:
	  ...
	  System.out.println("From Row: " + row + ", Column: " + column);
	  System.out.println();
		...
		for ( int row = 0; row < intArray.length; row++ ){
			for ( int column = 0; column < intArray[row].length; column++ ) {
				if (intArray[row][column] == value) {
					currentCount++;
					animate( row, column, currentCount );  // needed to create visual feedback
				}
			}
		}
		
		System.out.println("Number: " + value);
		System.out.println("Amount used: " + currentCount);
		System.out.println( );

		return currentCount; // replace this line with your return statement
   }
